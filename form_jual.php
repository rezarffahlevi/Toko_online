...
if (isset($_POST['run'])and($_POST['run']=="form2"))
{
	...
	if (!empty($_POST['qty'])and(!empty($_POST['harga'])))
	{
        ...
        $y=mysql_fetch_array($dbrg);
			...
			$subtotal    = ($_POST['harga']-$disc) * $_POST['qty'];
        ...
        if ($cekKode>0){
          ...
          $sql="UPDATE temp_jual_detail SET qty='$qtyy', harga_total='$subtotalX' WHERE barang_id='$_POST[pilih_barang]'";
      mysql_query($sql);
      ...
      $input="INSERT INTO temp_jual_detail(jual_id, barang_id, qty, harga_satuan, diskon, harga_total)
      VALUES('JL-$inc', '$dbuah[barang_id]', '$_POST[qty]','$_POST[harga]', '$_POST[diskon]', '$subtotal')";
      mysql_query($input);
      //update tabel stok
      $upstok="UPDATE stok SET qty='$sisa_qty' WHERE barang_id='$dbuah[barang_id]'";
      mysql_query($upstok);
...
<div class="header"><h1 style="float: left;">Transaksi Penjualan</h1>
        ...
        <table border="0" cellspacing="1" cellpadding="0">
          ...
          while ($dtmp=mysql_fetch_array($qtmp))
          {
		   $brg="SELECT * FROM  barang WHERE barang_id='$dtmp[barang_id]'";
			...
			$subtotal    = ($dtmp['harga_satuan']-$disc) * $dtmp['qty'];
			
          echo "
          <tr>
            <td>$dtmp[barang_id]</td>
            <td>$y[barang_nama]</td>
            <td>$y[barang_kategori]</td>
            <td>$dtmp[qty]</td>
            <td align=right>".digit($dtmp['harga_satuan'])."</td>
            <td align=right>".digit($dtmp['diskon'])."%</td>
...
echo 'Kode Produk : <select autofocus style="width:200px" id="states" name="pilih_barang" onchange="document.getElementById(\'barang_harga\').value = prdName[this.value]">';  
echo '<option>-------</option>';  
while ($row = mysql_fetch_array($result)) { 
 
    echo '<option value="' . $row['barang_id'] . '">' . $row['barang_id'] . '</option>'; 
