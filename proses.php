...
//pilih fungsi
	switch($proses){
		...
		{	$nama_unit=anti_injection($_POST["unit"]);
			$eceran=anti_injection($_POST["eceran"]);
			$keterangan=anti_injection($_POST["keterangan"]);
			$nama_tabel="unit";
			$values="'', '$nama_unit', '$eceran', '$keterangan'";
			$hal="data_unit";
			insert($nama_tabel,$values);
			break;
		}
				...
				if ($cek > 0){
 				$hal="form_data_master&kode=barang_insert&id=duplikat&data=$_POST[nmBarang]";	
					...
					if (move_uploaded_file($tmp, $path)) {
						...
						$nama_tabel="barang";
						$values="'$_POST[inc]', '$lokasi', '$barangKode', '$nmBarang', '$unit', '$kategori', '$harga_beli', '$harga_jual', '$diskon', '$foto'";
						insert($nama_tabel,$values);
						$hal="data_barang";
					}
					else {
		...
		case "jual_insert":
		{
			...
			mysql_query("INSERT INTO jual(inc, jual_id, tgl_jual, pelanggan_id, total, biaya_kirim)
			VALUES('$_POST[inc]','$_POST[jual_id]', '$_POST[tgl_jual]','$_POST[pelanggan_nama]', '$total','$biaya_kirim')");
			...
			while($dtmp=mysql_fetch_array($qtmp))
			{
				...
				mysql_query("INSERT INTO jual_detail(jual_id, barang_id,qty, harga_satuan, diskon, harga_total)
				VALUES('$_POST[jual_id]', '$dtmp[barang_id]','$dtmp[qty]','$dtmp[harga_satuan]', '$dtmp[diskon]', '$dtmp[harga_total]')");
			}
		...
		case "barang_update":
			{	
				...
				$diskon=anti_injection($_POST["diskon"]);
				$nama_tabel="barang";
				$values="barang_id='$_POST[Barang_Kode]', barang_nama='$nmBarang', barang_kategori='$kategori',harga_beli='$harga_beli',harga_jual='$harga_jual',diskon='$diskon',lokasi_id='$lokasi'";
				$kondisi="inc='$_POST[inc]'";
				$hal="data_barang";
				update($nama_tabel,$values,$kondisi);

				$sql="UPDATE stok SET barang_nama='$nmBarang' WHERE barang_id='$_POST[Barang_Kode]'";
			mysql_query($sql);
			...
			case "kategori_update":
			{
				...
				$sql="UPDATE kategori SET nama_kategori='$kategori' WHERE kategori_id='$id'";
			mysql_query($sql);
			...
			case "unit_update":
			{
				...
				$sql="UPDATE unit SET nama_unit='$unit', eceran = '$eceran', keterangan = '$keterangan' WHERE unit_id='$id'";
			mysql_query($sql);
		...
		case "supplier_update":
			{	
				...
				// Kalau data sudah ada
				if ($cek > 0){
 				$hal="form_ubah_data&kode=supplier_update&id=$_POST[supplier_id]&ids=duplikat&data=$_POST[nmSupplier]";	
				}
		...
		case "pelanggan_update":
			{	
				...
				// Kalau data sudah ada
				if ($cek > 0){
 				$hal="form_ubah_data&kode=pelanggan_update&id=$_POST[pelanggan_id]&ids=duplikat&data=$_POST[nmPelanggan]";	
				}
